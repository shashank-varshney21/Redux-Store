    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-10">
      {
        products.map(product=> (
          <div className='bg-white rounded-xl shadow-lg p-6 flex flex-col items-center text-center transition-transform duration-300 hover:scale-105 hover:shadow-2xl' key={product.id}>
            ...
            <button onClick={() => { HandleAdd(product) }} className='px-6 py-2 rounded-lg bg-indigo-600 text-white font-medium shadow-md hover:bg-indigo-700 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-indigo-400 transition-all duration-200 hover:cursor-pointer'>Add to Cart</button>
          </div>
        ))
      }
    </div>
